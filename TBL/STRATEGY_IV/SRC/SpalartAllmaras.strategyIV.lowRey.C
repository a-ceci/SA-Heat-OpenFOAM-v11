...
tmp<volScalarField> SpalartAllmaras<BasicMomentumTransportModel>::Diff
(
    const volScalarField& alphtFun
) const
{
    return volScalarField::New(typedName("Diff"), (this->nu())/Pr_ + alphtFun);
}
...
template<class BasicMomentumTransportModel>
tmp<volScalarField> SpalartAllmaras<BasicMomentumTransportModel>::chiTh() const
{
    return volScalarField::New(typedName("chiTh"), (nuTilda_/this->nu())/PrtInc_);
}
...
{
    const volScalarField chi2(typedName("chi2"), sqr(chi));
    return volScalarField::New(typedName("fv1"), chi2/(chi2 + sqr(Cv1_)));
}


template<class BasicMomentumTransportModel>
tmp<volScalarField> SpalartAllmaras<BasicMomentumTransportModel>::Cw2
(
    const volScalarField& chi
) const
{
    return volScalarField::New(typedName("Cw2"), Cw4_ + Cw5_/sqr(1.0+chi/40.0));
}


template<class BasicMomentumTransportModel>
tmp<volScalarField> SpalartAllmaras<BasicMomentumTransportModel>::alphtFun
(
    const volScalarField& chiTh
) const
{
    const volScalarField chi2Th(typedName("chi2Th"), sqr(chiTh));
    return volScalarField::New(typedName("alphtFun"), (nuTilda_/PrtInc_)*chi2Th/(chi2Th+sqr(CvTh_)));
}
...
(
    ...
    const volScalarField::Internal& Cw2
) const
{
    ...
    const volScalarField::Internal g(typedName("g"), r + Cw2*(pow6(r) - r)); // low Re
...
bool SpalartAllmaras<BasicMomentumTransportModel>::read()
{
    if (eddyViscosity<RASModel<BasicMomentumTransportModel>>::read())
    {
        ...
        Cw4_.readIfPresent(this->coeffDict());
        Cw5_.readIfPresent(this->coeffDict());
        Pr_.readIfPresent(this->coeffDict());
        PrtInc_.readIfPresent(this->coeffDict());
        CvTh_.readIfPresent(this->coeffDict());
...
void SpalartAllmaras<BasicMomentumTransportModel>::correct()
{
    ...
    const volScalarField Cw2(this->Cw2(chi));
    ...
    (
     ...
     ==
      ...
      - fvm::Sp(Cw1_*alpha()*rho()*fw(Stilda,Cw2)*nuTilda_()/sqr(y_), nuTilda_)
    ...
    if ( Pr_.value() != 0.0  )
    {
       ...
       const volScalarField chiTh(this->chiTh());
       const volScalarField alphtFun(this->alphtFun(chiTh));
       const volScalarField Diff(this->Diff(alphtFun));

       tmp<fvScalarMatrix> thetaEqn
       (
           fvm::ddt(theta_)
         + fvm::div(phi, theta_)
         - fvm::laplacian(Diff, theta_)
        ==
           fvModels.source(theta_)
       );

       thetaEqn.ref().relax();
       fvConstraints.constrain(thetaEqn.ref());
       solve(thetaEqn);
       fvConstraints.constrain(theta_);
       bound(theta_, dimensionedScalar(theta_.dimensions(), 0));
