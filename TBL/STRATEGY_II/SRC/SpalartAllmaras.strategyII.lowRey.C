...
tmp<volScalarField> SpalartAllmaras<BasicMomentumTransportModel>::Diff() const
{
    return volScalarField::New(typedName("Diff"), (this->nu())/Pr_ + (this->nut_)/PrtInc_ );
}
...
void SpalartAllmaras<BasicMomentumTransportModel>::correct()
{
    ...
    if ( Pr_.value() != 0.0  )
    {
       ...
       const volScalarField Diff(this->Diff());
